...
class TestCreateEvidence:
    ...
    def test_custom_evidence_id(self):
        item = create_evidence(
            source_path="/src", claim="test", evidence_id="custom-001",
        )
...
class TestValidateEvidence:
    def test_valid_passes(self):
        item = EvidenceItem(source_path="s", claim="c", evidence_type=EvidenceType.REPO_PATTERN)
...
class TestRejectUnsupportedClaims:
    def test_keeps_valid_items(self):
        items = (
            EvidenceItem(source_path="s1", claim="c1", evidence_type=EvidenceType.REPO_PATTERN),
            EvidenceItem(source_path="s2", claim="c2", evidence_type=EvidenceType.TEST_RESULT),
        )
    ...
    def test_rejects_invalid_items(self):
        items = (
            EvidenceItem(source_path="", claim="c1", evidence_type=EvidenceType.REPO_PATTERN),
            EvidenceItem(source_path="s2", claim="", evidence_type=EvidenceType.TEST_RESULT),
            EvidenceItem(source_path="s3", claim="c3", evidence_type=EvidenceType.REPO_PATTERN),
        )
        result = reject_unsupported_claims(items)
        assert len(result) == 1
...
class TestEvidenceCollector:
    ...
    def test_add_batch_skips_invalid(self, collector):
        items = (
            ...
            EvidenceItem(source_path="s3", claim="c3", evidence_type=EvidenceType.TEST_RESULT),
        )
    ...
    def test_summarize_by_agent(self, collector):
        collector.add(EvidenceItem(
            source_path="s1", claim="c1", agent_source="agent_a",
            evidence_type=EvidenceType.REPO_PATTERN,
        ))
        collector.add(EvidenceItem(
            source_path="s2", claim="c2", agent_source="agent_a",
            evidence_type=EvidenceType.REPO_PATTERN,
        ))
        collector.add(EvidenceItem(
            source_path="s3", claim="c3", agent_source="agent_b",
            evidence_type=EvidenceType.REPO_PATTERN,
        ))
